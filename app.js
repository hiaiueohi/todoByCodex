...
function renderSchedule() {
  ...
  for (let hour = 0; hour <= 24; hour += 1) {
    const slot = document.createElement('li'); slot.className = 'time-slot'; slot.style.setProperty('--slot-top', `${hour * 80}px`);
  ...
  if (selectedDate === dateToISO(new Date())) {
    ...
    if (minutes >= 0 && minutes <= 1440) {
      const line = document.createElement('li'); line.id = 'now-line'; line.className = 'now-line'; line.style.setProperty('--now-top', `${minutes * (4 / 3)}px`);
  ...
  scheduledWithLanes.forEach(({ task, lane }) => {
    ...
    row.style.setProperty('--task-top', `${visibleStart * (4 / 3)}px`); row.style.setProperty('--task-height', `${Math.max((visibleEnd - visibleStart) * (4 / 3), 52)}px`); row.style.left = `calc(66px + ${lane * 12}%)`; row.style.right = '4px'; row.style.zIndex = String(lane + 1);
...
function enableScheduleDrag(row, task) {
  ...
  row.addEventListener('pointermove', event => {
    ...
    const movedMinutes = Math.round(((event.clientY - dragging.startY) / (4 / 3)) / 5) * 5;
    const nextStart = Math.max(0, Math.min(1440 - dragging.duration, dragging.start + movedMinutes));
    dragging.nextStart = nextStart; row.style.setProperty('--task-top', `${nextStart * (4 / 3)}px`);
...
function updateNowLine() {
  ...
  line.style.setProperty('--now-top', `${minutes * (4 / 3)}px`);
